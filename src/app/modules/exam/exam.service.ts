...
const getExamFromDB = async (examId: string): Promise<IExam> => {
  const exam = await Exam.findById(examId).populate("subjects.subjectTeacher").populate("students");
  if (!exam) {
...
const updateExamInDB = async (examId: string, payload: Partial<IExam>): Promise<IExam> => {
  const updatedExam = await Exam.findByIdAndUpdate(examId, payload, { new: true })
